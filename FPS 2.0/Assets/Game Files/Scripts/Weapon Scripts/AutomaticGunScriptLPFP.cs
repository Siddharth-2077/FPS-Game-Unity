...
public class AutomaticGunScriptLPFP : MonoBehaviour {
	...
	// CUSTOM PARAMETERS:
	[Header("Weapon Recoil Components:")]
	[Space(10f)]
	[SerializeField] private Transform rigTransform = null;

	[Header("Weapon Recoil Parameters:")]
	[Space(10f)]
	[Tooltip(" Recoil without Aim: \n X = RecoilX\n Y = RecoilY\n Z = RecoilZ")]
	[SerializeField] private Vector3 recoil;
	[Tooltip(" Recoil while Aiming: \n X = RecoilX\n Y = RecoilY\n Z = RecoilZ")]
	...
	private void Update () {
		...
		if (settingsMenu.gamePaused == false) {
			...
			rigTransform.localRotation = Quaternion.Euler(currentrotation);
	...
	// CUSTOM DEFINITION: [Weapon recoil]
	public void SimulateRecoil(bool aiming) {
		if (aiming == true) {   // Aim Recoil
	...
	private void Reload () {
		...
		else 
		{
			...
			// Custom: If ammo is full, don't reload
			if (currentAmmo != ammo) {
